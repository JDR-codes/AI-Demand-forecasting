...
# Import AI modules
from fastapi_app.ai.arima import forecast as arima_forecast, find_peaks
    ...
    def _get_series(db: Session, job: ForecastJob) -> Optional[pd.Series]:
        ...
        if job.upload_id:
            ...
            if upload and upload.file_path:
                try:
                    series = prepare_series(path=upload.file_path)
        ...
        try:
            ...
            if records:
                ...
                values = [r.demand for r in records if r.demand is not None]
                if dates and values:
