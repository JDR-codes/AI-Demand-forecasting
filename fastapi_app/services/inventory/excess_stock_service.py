...
class ExcessStockService:
    ...
    def determine_excess_level(days_inventory_on_hand: float) -> str:
        ...
        if days_inventory_on_hand >= EXCESS_LEVEL_THRESHOLDS["critical"]:
            return "critical"
        elif days_inventory_on_hand >= EXCESS_LEVEL_THRESHOLDS["high"]:
            return "high"
        elif days_inventory_on_hand >= EXCESS_LEVEL_THRESHOLDS["medium"]:
            return "medium"
        else:
            return "low"
    ...
    def identify_excess_stock(db: Session) -> Dict[str, Any]:
        ...
        for warehouse_inv in all_inventory:
            ...
            if days_inventory_on_hand < EXCESS_LEVEL_THRESHOLDS["low"]:
                continue
